...
local function getServerEnv(name)
	local thisEnv = ServerRemFolder:FindFirstChild(name)
	...
	if thisEnv then 
		...
		newBridge.Name = name
		newBridge.Parent = ServerRemFolder
...
local function getClientEnv(name)
	local thisEnv = ClientRemFolder:FindFirstChild(name)
	...
	if thisEnv then 
		...
		local newBridge = Instance.new("RemoteEvent")
		newBridge.Name = name
		newBridge.Parent = ClientRemFolder
...
do
	...
	do
		...
		pL:Hook(function(dataPath, value)
			envUpdatedEvent:FireAllClients(dataPath, value)
		end)
	end
	...
	do
		...
		pL:Hook(function(dataPath, value)
			for _, player in ipairs(Subscribers) do
				envUpdatedEvent:FireClient(player, dataPath, value)
			end
		end)
	end
